...
WHITE_CATS=[line.decode('utf-8').rstrip('\n') for line in open(CURR_PATH + '/whitecats.txt') if not line.startswith('#')]
BLACK_REGEX=[line.decode('utf-8').rstrip('\n') for line in open(CURR_PATH + '/blacklist.txt') if not line.startswith('#')]
...
if (len(sys.argv) != 2):
    ...
    with gzip.open(sys.argv[1], 'rb') as chanFile:
        ...
        filteredChanJson = [x for x in chanJson["channels"] if
                (x['cat'] in WHITE_CATS) and
                not any(r for r in BLACK_REGEX if re.search(r, x['name'], re.IGNORECASE))]
